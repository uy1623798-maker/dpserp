...
export default function SchoolLogin(){
  const[role,setRole]=useState('STUDENT'),[loginId,setLoginId]=useState(ids.STUDENT),[dob,setDob]=useState(dobs.STUDENT),[error,setError]=useState(''),[busy,setBusy]=useState(false),[showFounder,setShowFounder]=useState(false),nav=useNavigate();
  useEffect(()=>{const reveal=setTimeout(()=>setShowFounder(true),3500),hide=setTimeout(()=>setShowFounder(false),10500);return()=>{clearTimeout(reveal);clearTimeout(hide)}},[]);
  ...
  return <main className="login rolelogin">
    {showFounder?<aside className="motivationpop founderpop" role="status" aria-live="polite"><span className="founderphoto"><img src="/utkarsh-founder.jpeg" alt="Utkarsh, Founder of WeTakeFwd"/></span><div><small>A message from the founder</small><strong>Hi, I’m Utkarsh — Founder of WeTakeFwd. Dream boldly and keep moving forward! 🚀</strong></div><button type="button" aria-label="Close founder message" onClick={()=>setShowFounder(false)}><X/></button></aside>:null}
